...
功能权限：基于 RBAC 的模型，通过用户、角色、权限以及资源来控制权限，检查用户能做什么。在 OAuth2 中对应 Authorities 参数。

功能权限的设计要素是：我是谁？我能做什么事情？

数据权限：数据权限需要基于功能权限之上，解决的是该功能下能访问的数据控制。比如，某个用户能列出某个机构的保单列表，那么首先需要拥有**列出**功能权限。在 OAuth2 中对应 Scope 参数，刚好匹配了该英文单词的含义，即数据范围。

数据权限的要素：数据 scope 划分维度是什么？怎么标记用户可以拥有这些数据维度【设计难点】。可用的维度参考：组织机构编码、租户编码、店铺编码等较为粗粒度的数据管辖单位，并且是通用的数据管辖范围。

有一些难题：我创建的订单、文章，我协作创建的订单、画布，我待审核的审批单，对手交易数据（合同的双方）。这些算不算同样的数据权限？**这些非通用数据权限控制，不要纳入通用的数据权限控制机中，应该作为业务条件使用。**

数据权限设计的难点在于，通用数据权限配置、控制和业务条件有时候难以找出边界，业务也很难说清楚。

权限模型图：

![](./java-solution-webinar-5/permission-model.jpg)

**鉴权拦截在哪里做？**

无论是功能权限还是数据权限，需要设计检查点，而检查点又**取决于我们的架构分层**。

比如，如果有应用层，很容易实现在应用层的功能权限检查；但是数据权限又必然会侵入领域逻辑。检查点由三个元素构成：**权限的主体（用户）、客体（资源）、条件。**

对于功能权限来说，可以放到应用层，因为只有应用层是面向用例，而领域层面向通用能力。

权限拦截图：

![](./java-solution-webinar-5/permission-check-points.png)

**原则：权限拦截点后端必做，前端做只是为了用户体验。**

架构师的目标心得：通过预判和充分的设计，消除系统的不确定性，降低系统成本和防止返工。
...
链接: https://pan.baidu.com/s/1UFlKVeX45QFfxgvsTE0J3w?pwd=cfng 提取码: cfng 复制这段内容后打开百度网盘手机App，操作更方便哦

2022-11-26 授权和认证 part3

链接: https://pan.baidu.com/s/1Ko2-jjZzIyPGoBrdg4UNHg?pwd=cg8e 提取码: cg8e 复制这段内容后打开百度网盘手机App，操作更方便哦
