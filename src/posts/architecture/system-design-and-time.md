...
虽然以 GMT 时区作为参考基线，UT 却不完全等同于 GMT 时间。现代世界标准时是 UTC（Coodinated Universal Time），即协调世界时。UTC 基于 GMT 并结合各国时间同步协调和原子钟精确测算得到的标准时间，被用于更高精度场景。在军事上，协调时间会使用 Z 作为代号，在无线电中使用 "Zulu" 作为呼号，所以协调世界时有时候被叫做 "Zulu time"。
